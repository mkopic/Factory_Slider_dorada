...
$(".next").click(function(){
   if(counter <= sliderImgTop.length && counter <= sliderImgBottom.length){  
    sliderTop.animate({left: "+=" +sizeTop});
    sliderBottom.animate({left: "+=" +sizeBottom});
    sizeTop = sliderImgTop[counter].clientWidth + 10;       
    sizeBottom = sliderImgBottom[counter].clientWidth + 10;
    counter++;     
    if(counter < sliderImgTop.length && counter < sliderImgBottom.length) return;
    else{ 
      counter = 0;
      cloneSliderTop = sliderImgTop.clone().appendTo(sliderTop);
      cloneSlideBottom = sliderImgBottom.clone().appendTo(sliderBottom);
      cloneSliderTop.animate({left: "+=" +sizeTop});
      cloneSliderBottom.animate({left: "+=" +sizeBottom});      
    }     
  }
})
$(".prev").click(function(){ 
  if (counter > 1){
    ...
    imagePrevTop.animate({left: "-=" +sizePrevTop}); 
    imagePrevBottom.animate({left: "-=" +sizePrevBottom});  
  }  
  });
});
